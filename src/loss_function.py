...
def hash_labels(labels):
    ...
    return torch.LongTensor(hash_labels).to(labels.device)
...
def TripletSemiHardLoss(y_true, y_pred, margin=1.0):
    ...
    pdist_matrix = pairwise_distance_torch(embeddings).to(device)
...
class FocalLoss(torch.nn.CrossEntropyLoss):
    ...
    def __init__(self, gamma, alpha=None, ignore_index=-1, reduction='mean'):
    ...
    def forward(self, input_, target):
        ...
        input_prob = torch.gather(torch.nn.functional.softmax(input_, 1), 1, target.unsqueeze(1))
