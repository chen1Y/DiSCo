...
class Discriminator_v2(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512, attr_num=None):
        ...
        self.condition_2 = ConditionEncoder(attr_num, 256, nfc[512] // 2, '2d')
        self.condition_4 = ConditionEncoder(attr_num, 128, nfc[256] // 2, '2d')
        self.condition_8 = ConditionEncoder(attr_num, 64, nfc[128] // 2, '2d')
        self.condition_16 = ConditionEncoder(attr_num, 32, nfc[64] // 2, '2d')
        self.condition_32 = ConditionEncoder(attr_num, 16, nfc[32] // 2, '2d')
        self.condition_64 = ConditionEncoder(attr_num, 8, nfc[16] // 2, '2d')

        self.down_4 = DownBlockComp(nfc[512] + nfc[512] // 2, nfc[256])
        self.down_8 = DownBlockComp(nfc[256] + nfc[256] // 2, nfc[128])
        self.down_16 = DownBlockComp(nfc[128] + nfc[128] // 2, nfc[64])
        self.down_32 = DownBlockComp(nfc[64] + nfc[64] // 2, nfc[32])
        self.down_64 = DownBlockComp(nfc[32] + nfc[32] // 2, nfc[16])

        self.rf_big = nn.Sequential(
            conv2d(nfc[16] + nfc[16] // 2, nfc[8], 1, 1, 0, bias=False),
            batchNorm2d(nfc[8]), nn.LeakyReLU(0.2, inplace=True),
            conv2d(nfc[8], 1, 4, 1, 0, bias=False))

        self.se_2_16 = SEBlock(nfc[512] + nfc[512] // 2, nfc[64] + nfc[64] // 2)
        self.se_4_32 = SEBlock(nfc[256] + nfc[256] // 2, nfc[32] + nfc[32] // 2)
        self.se_8_64 = SEBlock(nfc[128] + nfc[128] // 2, nfc[16] + nfc[16] // 2)
        ...
        self.condition_4s = ConditionEncoder(attr_num, 64, nfc[256] // 2, '2d')
        self.condition_8s = ConditionEncoder(attr_num, 32, nfc[128] // 2, '2d')
        self.condition_16s = ConditionEncoder(attr_num, 16, nfc[64] // 2, '2d')
        self.condition_32s = ConditionEncoder(attr_num, 8, nfc[32] // 2, '2d')

        self.down_8s = DownBlock(nfc[256] + nfc[256] // 2, nfc[128])
        self.down_16s = DownBlock(nfc[128] + nfc[128] // 2, nfc[64])
        self.down_32s = DownBlock(nfc[64] + nfc[64] // 2, nfc[32])

        self.rf_small = conv2d(nfc[32] + nfc[32] // 2, 1, 4, 1, 0, bias=False)

        self.decoder_big = SimpleDecoder(nfc[16] + nfc[16] // 2, nc)
        self.decoder_part = SimpleDecoder(nfc[32] + nfc[32] // 2, nc)
        self.decoder_small = SimpleDecoder(nfc[32] + nfc[32] // 2, nc)

    def forward(self, imgs, y,label,part=None):
        if type(imgs) is not list:
            imgs = [F.interpolate(imgs, size=self.im_size), F.interpolate(imgs, size=128)]
...
class Generator_v2(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024, attr_num=None, init_condition_dim=256):
        ...
        self.condition_embedd = ConditionEncoder(attr_num, 1, init_condition_dim, '1d')

        self.init = InitLayer(nz + init_condition_dim, channel=nfc[4])  # => (1024, 4, 4)

        self.feat_8 = UpBlockComp(nfc[4], nfc[8])  # => (512, 8, 8)
        self.feat_16 = UpBlock(nfc[8] * 2, nfc[16])  # => (256, 16, 16)
        self.feat_32 = UpBlockComp(nfc[16], nfc[32])  # => (128, 32, 32)
        self.feat_64 = UpBlock(nfc[32] * 2, nfc[64])  # => (128, 64, 64)
        self.feat_128 = UpBlockComp(nfc[64], nfc[128])  # => (64, 128, 128)
        self.feat_256 = UpBlock(nfc[128] * 2, nfc[256])  # => (32, 256, 256)

        self.condition_8 = ConditionEncoder(attr_num, 8, nfc[8], '2d')
        self.condition_32 = ConditionEncoder(attr_num, 32, nfc[32], '2d')
        self.condition_128 = ConditionEncoder(attr_num, 128, nfc[128], '2d')

        self.se_64 = SEBlock(nfc[4], nfc[64])
        self.se_128 = SEBlock(nfc[8] * 2, nfc[128] * 2)
        self.se_256 = SEBlock(nfc[16], nfc[256])

        self.to_128 = conv2d(nfc[128] * 2, nc, 1, 1, 0, bias=False)
        self.to_big = conv2d(nfc[im_size], nc, 3, 1, 1, bias=False)
    ...
    def forward(self, code, y):
        ...
        conditional_code = torch.cat([code, con_embed], dim=1)
...
class ConditionEncoder(nn.Module):
    def __init__(self, attr_num, con_hw, con_ch, type='2d'):
        super(ConditionEncoder, self).__init__()
        self.attr_num = attr_num
        self.con_hw = con_hw
        self.con_ch = con_ch
        if type == '2d':
            self.nembedding = con_hw ** 2
            self.up = nn.Conv2d(len(attr_num), con_ch, 1, 1, 0, bias=False)
        else:
            self.nembedding = con_ch
            self.up = nn.Conv2d(con_ch * len(attr_num), con_ch, 1, 1, 0, bias=False)
        self.type = type
        embed_list = []
    ...
    def forward(self, label):
        ...
        if self.type == '1d':
            ...
            condition_embedd = self.up(
                torch.cat(condition_embedd_list, dim=1).view(b, -1, 1, 1)
            )
            condition_embedd = condition_embedd.view(b,-1)
        elif self.type == '2d':
            b = condition_embedd_list[0].shape[0]
            condition_embedd = self.up(
                torch.cat(condition_embedd_list, dim=1).view(b, -1, self.con_hw, self.con_hw)
            )
...
if __name__ == '__main__':
    ...
    g = Generator_v2(ngf=128, nz=shape, im_size=256, attr_num=attr_num, init_condition_dim=256)
    # print(label)
    # d = Discriminator_v2(ndf=128, im_size=256, attr_num=attr_num)
    summary(g, input_data=[code, label], col_names=['input_size', 'output_size', 'params_percent'])
    # summary(d, input_data=[pic, label], col_names=['input_size', 'output_size', 'params_percent'])
