...
class DataQuery(data.Dataset):
    ...
    def __init__(self, file_root,  img_root_path, ref_ids,  query_inds, img_transform=None,
        ...
        self.img_data, self.label_data, self.ref_idxs, self.query_inds, self.attr_num, self.gt_data = self._load_dataset()
    ...
    def _load_dataset(self):
        ...
        gt_data = np.loadtxt(os.path.join(self.file_root,"gt_test.txt"),dtype=int)
        ...
        return img_data, label_data, ref_idxs, query_inds, attr_num, gt_data
    ...
    def __getitem__(self, index):
        ...
        return img, indicator,gt,get_idx_label(gt,self.attr_num)
...
class DataRealGAN(data.Dataset):
    """
    Load real images for discriminator training
    """
    def __init__(self, file_root, img_root_path, img_transform=None):
        self.file_root = file_root
        self.img_transform = img_transform
        self.img_root_path = img_root_path
    ...
    def _load_dataset(self):
        ...
        label_data = np.loadtxt(os.path.join(self.file_root, "labels_train.txt" ), dtype=int)
        assert len(img_data) == label_data.shape[0]
    ...
    def __getitem__(self, index):
        ...
        return img, label_vector,get_idx_label(label_vector, self.attr_num)


class DataCodeGAN(data.Dataset):
    """
    Load pre-generated image feature as latent code for generator training
    """
    def __init__(self,file_root,img_root_path,img_transform):
        self.file_root = file_root
        self.img_transform = img_transform
        self.img_root_path = img_root_path
    ...
    def _load_dataset(self):
        # code_data = np.load(os.path.join(self.file_root,'latent_vector.npy'))
        with open(os.path.join(self.file_root, "imgs_train.txt")) as f:
            img_data = f.read().splitlines()
        label_data = np.loadtxt(os.path.join(self.file_root, "labels_train.txt" ), dtype=int)
        # assert code_data.shape[0] == label_data.shape[0]
    ...
    def __getitem__(self, index):
        ...
        if self.img_transform:
            img = self.img_transform(img)
        one_hot_label = self.label_data[index]
        return img, one_hot_label,get_idx_label(one_hot_label, self.attr_num)
...
#             self.img_transform = transforms.ToTensor()
#
#         self.img_data, self.code_data, self.label_data, self.attr_num = self._load_dataset()
#
#     def _load_dataset(self):
#         code_data = np.load(os.path.join(self.file_root, 'latent_vector.npy'))
#         with open(os.path.join(self.file_root, "imgs_train.txt")) as f:
...
#         attr_num = np.loadtxt(os.path.join(self.file_root, "attr_num.txt"), dtype=int)
#
#         return img_data, code_data, label_data, attr_num
#
#     def __len__(self):
#         return self.code_data.shape[0]
#
#     def __getitem__(self, index):
#         img = Image.open(os.path.join(self.img_root_path, self.img_data[index]))
#         img = img.convert('RGB')
...
#             img = self.img_transform(img)
#         one_hot_label = self.label_data[index]
#         return img, self.code_data[index], one_hot_label, get_idx_label(one_hot_label, self.attr_num)